...
export class StreamBridge {
  ...
  private connectUpstream(): void {
    ...
    // Support optional custom voice or model
      ...
      switch (msg.type) {
        ...
        case 'ping':
          ...
          if (this.upstreamWs && this.upstreamWs.readyState === WebSocket.OPEN) {
            this.upstreamWs.send(JSON.stringify({ pong: { event_id: msg.ping_event.event_id } }));
          }
          break;
      }
    } catch {
  ...
  private async executeFailover(reason: string, elapsedMs: number): Promise<void> {
    ...
    await this.twilioClient.redirectCall(this.callSid, fallbackUrl.toString());
